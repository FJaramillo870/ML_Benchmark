...
def get_cifar10_loaders(batch_size=8):
    ...
    # 1. THE TRAINING SHIELD (Data Augmentation)
    # We alter the images slightly every single time they are loaded
    train_transform = transforms.Compose([
        # Adds a 4-pixel black border, then randomly crops it back down to 32x32
        # This physically shifts the image up/down/left/right by a few pixels
        transforms.RandomCrop(32, padding=4),

        # 50% chance to mirror the image left-to-right
    ...
    # 2. THE EVALUATION PIPELINE (Strict & Static)
    # The final exam must NEVER be flipped or altered!
    test_transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(0.5, 0.5)
    ])

    # Apply the augmented transform to the training fuel
    train_dataset = datasets.CIFAR10(root="data", train=True, download=True, transform=train_transform)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

    # Apply the static transform to the test fuel
